...
describe('LoginRouteGuard', () => {
  ...
  describe('when loginMethod is a social provider', () => {
    it('should render SocialAutoLoginRedirect for google', () => {
      mockSearchParams('loginMethod=google')
      const { getByTestId } = render(<LoginRouteGuard />)
      expect(getByTestId('social-auto-login-redirect')).toBeInTheDocument()
      expect(getByTestId('social-auto-login-redirect')).toHaveAttribute('data-connection-type', 'google')
    })

    it('should render SocialAutoLoginRedirect for discord', () => {
      mockSearchParams('loginMethod=discord')
      const { getByTestId } = render(<LoginRouteGuard />)
      expect(getByTestId('social-auto-login-redirect')).toBeInTheDocument()
      expect(getByTestId('social-auto-login-redirect')).toHaveAttribute('data-connection-type', 'discord')
    })

    it('should render SocialAutoLoginRedirect for apple', () => {
      mockSearchParams('loginMethod=apple')
      const { getByTestId } = render(<LoginRouteGuard />)
      expect(getByTestId('social-auto-login-redirect')).toBeInTheDocument()
      expect(getByTestId('social-auto-login-redirect')).toHaveAttribute('data-connection-type', 'apple')
    })

    it('should render SocialAutoLoginRedirect for x', () => {
      mockSearchParams('loginMethod=x')
      const { getByTestId } = render(<LoginRouteGuard />)
      expect(getByTestId('social-auto-login-redirect')).toBeInTheDocument()
      expect(getByTestId('social-auto-login-redirect')).toHaveAttribute('data-connection-type', 'x')
    })

    it('should not render LoginPage for google', () => {
      mockSearchParams('loginMethod=google')
  ...
  describe('when loginMethod is not a social provider', () => {
