...
// The two ends of the credits ramp, as the shop defines them (`app/src/styles/theme.ts`: dclRed and
// flareAmber). Written here rather than taken from the Decentraland theme because they are the currency's
// colours, not the site's, and they have to stay the same on every surface that shows the mark.
const CREDITS_RAMP_START = '#ff2d55'
const CREDITS_RAMP_END = '#ffbc5b'

/**
 * The credits mark: the same filled octagon-and-C the shop puts next to a price (its `CreditMarkIcon`) and
 * the Explorer shows in the client, drawn from the same path data and the same two-stop ramp.
 *
 * Inline rather than an imported file on purpose. It is one glyph, it must not change colour with the
 * theme — a currency mark that turns grey in a light palette stops being the currency — and shipping it as
 * markup keeps auth from carrying a binary that could drift from the shop's copy without anyone noticing.
 *
 * `useId` keeps the gradient id unique: two marks on one screen would otherwise share a def, and the second
 * would silently retarget the first.
 */
const CreditsMark = ({ size = 28, ...props }: { size?: number } & SVGProps<SVGSVGElement>) => {
  const gradientId = `credits-mark-${useId()}`
  return (
    <svg viewBox="0 0 16.9985 16.9985" width={size} height={size} fill="none" aria-hidden focusable="false" {...props}>
      <defs>
        <linearGradient id={gradientId} x1="-0.513692" y1="8.51179" x2="29.4301" y2="5.69069" gradientUnits="userSpaceOnUse">
          <stop stopColor={CREDITS_RAMP_START} />
          <stop offset="1" stopColor={CREDITS_RAMP_END} />
        </linearGradient>
      </defs>
      <path
...
export { CREDITS_RAMP_END, CREDITS_RAMP_START, CreditsMark }
